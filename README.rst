...
- Run from source: ``npm run start`` (runs ``src/app.ts`` directly using `tsx`)
...
----------------------------------
Use the included unit file at ``scripts/loupe-flightdeck.service``:

::

   mkdir -p ~/.config/systemd/user
   cp scripts/loupe-flightdeck.service ~/.config/systemd/user/
   # Optional: edit ExecStart in the copied unit file to use absolute paths
   systemctl --user daemon-reload
   systemctl --user enable --now loupe-flightdeck.service
   journalctl --user -u loupe-flightdeck.service -f

To run it even when you are not logged in (optional):
...
- Linux permission issue: copy ``scripts/50-loupedeck.rules`` to be under ``/etc/udev/rules.d`` and then ``sudo udevadm control --reload-rules && sudo udevadm trigger``. In Linux, install the font ``font.ttf`` to your system.
